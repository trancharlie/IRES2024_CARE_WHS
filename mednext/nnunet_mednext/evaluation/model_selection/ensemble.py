...
from nnunet_mednext.paths import network_training_output_dir, preprocessing_output_dir
from nnunet_mednext.postprocessing.connected_components import determine_postprocessing


def merge_triple(args):
...
def ensemble_triple(training_output_folder1, training_output_folder2, training_output_folder3, output_folder, task, validation_folder, folds, allow_ensembling: bool = True):
    ...
    for f in folds:
        ...
        for validation_folder_net in [validation_folder_net1, validation_folder_net2, validation_folder_net3]:
            if not isdir(validation_folder_net):
                raise AssertionError("Validation directory missing: %s. Please rerun validation with `nnUNet_train CONFIG TRAINER TASK FOLD -val --npz`" % validation_folder_net)
            if not isfile(join(validation_folder_net, 'summary.json')):
                raise AssertionError("Validation directory incomplete: %s. Please rerun validation with `nnUNet_train CONFIG TRAINER TASK FOLD -val --npz`" % validation_folder_net)
        ...
        patient_identifiers3_npz = [i[:-4] for i in subfiles(validation_folder_net3, False, None, 'npz', True)]
        ...
        patient_identifiers3_nii = [i[:-7] for i in subfiles(validation_folder_net3, False, None, suffix='nii.gz', sort=True) if not i.endswith("noPostProcess.nii.gz") and not i.endswith('_postprocessed.nii.gz')]

        for patient_identifiers_npz, patient_identifiers_nii, validation_folder_net in zip([patient_identifiers1_npz, patient_identifiers2_npz, patient_identifiers3_npz], [patient_identifiers1_nii, patient_identifiers2_nii, patient_identifiers3_nii], [validation_folder_net1, validation_folder_net2, validation_folder_net3]):
            if not all([i in patient_identifiers_npz for i in patient_identifiers_nii]):
                raise AssertionError("Missing npz files in folder %s. Please run the validation for all models and folds with the '--npz' flag." % (validation_folder_net))

            patient_identifiers_npz.sort()

#        assert all([i == j for i, j in zip(patient_identifiers1_npz, patient_identifiers2_npz, patient_identifiers3_npz)]), "npz filenames do not match. This should not happen."
        assert all(i == j == k for i, j, k in zip(patient_identifiers1_npz, patient_identifiers2_npz, patient_identifiers3_npz)), "npz filenames do not match. This should not happen."
        ...
        for p in patient_identifiers1_npz:
            ...
            files3.append(join(validation_folder_net3, p + '.npz'))
    ...
    p.map(merge_triple, zip(files1, files2, files3, property_files, out_files))
    p.close()
    p.join()
...
def ensemble_double(training_output_folder1, training_output_folder2, output_folder, task, validation_folder, folds, allow_ensembling: bool = True):
    print("\nEnsembling folders\n", training_output_folder1, "\n", training_output_folder2)
    ...
    p = Pool(default_num_threads)
    p.map(merge_double, zip(files1, files2, property_files, out_files))
    p.close()
    p.join()

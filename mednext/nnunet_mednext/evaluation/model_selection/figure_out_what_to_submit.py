...
from nnunet_mednext.evaluation.add_mean_dice_to_json import foreground_mean
from nnunet_mednext.evaluation.evaluator import evaluate_folder
from nnunet_mednext.evaluation.model_selection.ensemble import ensemble_double, ensemble_triple
from nnunet_mednext.paths import network_training_output_dir
import numpy as np
from subprocess import call
from nnunet_mednext.postprocessing.consolidate_postprocessing import consolidate_folds, collect_cv_niftis
from nnunet_mednext.utilities.folder_names import get_output_folder_name
from nnunet_mednext.paths import default_cascade_trainer, default_trainer, default_plans_identifier
    ...
    for t in tasks:
        ...
        for m in models:
            ...
            if t not in id_task_mapping.keys():
                #print('OUTPUT FOLDER NAMES...', get_output_folder_name(m))
                task_name = find_task_name(get_output_folder_name(m, overwrite_training_output_dir = './nnUNet_trained_models/nnUNet/'), t)
                id_task_mapping[t] = task_name

            output_folder = get_output_folder_name(m, id_task_mapping[t], trainer, pl, overwrite_training_output_dir = './nnUNet_trained_models/nnUNet/')
...
###################################################################################################
            print("\nI will now ensemble double combinations of the following models:\n", valid_models)
            if len(valid_models) > 1:
                for m1, m2 in combinations(valid_models, 2):
                    ...
                    network1_folder = get_output_folder_name(m1, id_task_mapping[t], trainer_m1, pl, overwrite_training_output_dir = './nnUNet_trained_models/nnUNet/')
                    network2_folder = get_output_folder_name(m2, id_task_mapping[t], trainer_m2, pl, overwrite_training_output_dir = './nnUNet_trained_models/nnUNet/')

                    #print("ensembling", network1_folder, network2_folder)
                    ensemble_double(network1_folder, network2_folder, output_folder_base, id_task_mapping[t], validation_folder, folds, allow_ensembling=not disable_postprocessing)
                    # ensembling will automatically do postprocessingget_foreground_mean
                    #print("I MARKED THE ENSEMBLE DONE HERE")
                   ...
                   # print("GET MEAN FOREGROUND COMPLETED")
...
#####################################################################################################
            ...
            print("\nI will now ensemble triple combinations of the following models:\n", valid_models)
            if len(valid_models) > 2:  # Check if there are at least three valid models
                for m1, m2, m3 in combinations(valid_models, 3):  # Iterate over combinations of three models

                    # Determine trainers based on model names (similar to existing logic)
                    trainer_m1 = trc if m1 == "3d_cascade_fullres" else tr
                    trainer_m2 = trc if m2 == "3d_cascade_fullres" else tr
                    trainer_m3 = trc if m3 == "3d_cascade_fullres" else tr

                    # Generate ensemble name
                    ensemble_name = "ensemble_" + m1 + "__" + trainer_m1 + "__" + pl + "--" + \
                                    m2 + "__" + trainer_m2 + "__" + pl + "--" + \
                                    m3 + "__" + trainer_m3 + "__" + pl

                    # Create output folder structure
                    output_folder_base = join(network_training_output_dir, "ensembles", id_task_mapping[t], ensemble_name)
                    maybe_mkdir_p(output_folder_base)

                    # Get output folders for individual networks
                    network1_folder = get_output_folder_name(m1, id_task_mapping[t], trainer_m1, pl, overwrite_training_output_dir = './nnUNet_trained_models/nnUNet/')
                    network2_folder = get_output_folder_name(m2, id_task_mapping[t], trainer_m2, pl, overwrite_training_output_dir = './nnUNet_trained_models/nnUNet/')
                    network3_folder = get_output_folder_name(m3, id_task_mapping[t], trainer_m3, pl, overwrite_training_output_dir = './nnUNet_trained_models/nnUNet/')
                    print(f'CHECKING HERE....{m1}')
                    print(f'CHECKING HERE....{m2}')
                    print(f'CHECKING HERE....{m3}')
                    # Print information about ensembling process
                    print("ensembling", network1_folder, network2_folder, network3_folder)

                    # Perform ensembling (assuming 'ensemble' function exists)
                    ensemble_triple(network1_folder, network2_folder, network3_folder, output_folder_base, id_task_mapping[t], validation_folder, folds, allow_ensembling=not disable_postprocessing)

                    # Get results of ensemble
                    results[ensemble_name] = get_mean_foreground_dice(join(output_folder_base, "ensembled_raw", "summary.json"))

                    # Perform postprocessing if not disabled
        ...
        for k, v in results.items():
            if v == best:
                ...
                if k.startswith("ensemble"):
                    tmp = k[len("ensemble_"):]
                    model1, model2, model3 = tmp.split("--")
                    m1, t1, pl1 = model1.split("__")
                    m2, t2, pl2 = model2.split("__")
                    m3, t3, pl3 = model3.split("__")
                    ...
                    predict_str += "nnUNet_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL2 -tr " + tr + " -ctr " + trc + " -m " + m2 + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + " -z\n"
                    predict_str += "nnUNet_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL3 -tr " + tr + " -ctr " + trc + " -m " + m3 + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + " -z\n"

                    if not disable_postprocessing:
                        predict_str += "nnUNet_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 OUTPUT_FOLDER_MODEL3 -o OUTPUT_FOLDER -pp " + join(network_training_output_dir, "ensembles", id_task_mapping[t], k, "postprocessing.json") + "\n"
                    else:
                        predict_str += "nnUNet_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 OUTPUT_FOLDER_MODEL3 -o OUTPUT_FOLDER\n"
                else:

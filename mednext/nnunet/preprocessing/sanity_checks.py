...
def verify_dataset_integrity(folder):
    ...
    :return:
    """
    assert isfile(os.path.join(folder, "dataset.json")), "There needs to be a dataset.json file in folder, folder=%s" % folder
    assert isdir(os.path.join(folder, "imagesTr")), "There needs to be a imagesTr subfolder in folder, folder=%s" % folder
    assert isdir(os.path.join(folder, "labelsTr")), "There needs to be a labelsTr subfolder in folder, folder=%s" % folder
    ...
    ## check training set
    nii_files_in_imagesTr = subfiles((os.path.join(folder, "imagesTr")), suffix=".nii.gz", join=False)
    nii_files_in_labelsTr = subfiles((os.path.join(folder, "labelsTr")), suffix=".nii.gz", join=False)
    ...
    if len(expected_test_identifiers) > 0:
        ...
        nii_files_in_imagesTs = subfiles((os.path.join(folder, "imagesTs")), suffix=".nii.gz", join=False)
